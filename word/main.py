...
def extract_audio(filepath, channels=1, rate=16000):
    ...
    os.mkdir('temp')
    print((os.path.split(filepath)[-1])[:-3])
    ...
    # subprocess.check_output(command, stdin=open(os.devnull), shell=use_shell)     # linux
    subprocess.check_output(command)                                                # windows
...
class SpeechRecognizer(object):
    def __init__(self, api_key, secret_key, rate, audioformat, retries=3):
        ...
        self.dev_pid = 1536  # 1537 denotes recognition of Putonghua, uses input method model. 1536 denotes recognition of Putonghua, uses search model
        self.cuid = '123456PYTHON'
        self.asr_url = 'http://vop.baidu.com/server_api'
        self.token_url = 'http://openapi.baidu.com/oauth/2.0/token'
        # If the authorization authentication return value does not have this string, then the user application does not have asr enabled, need to open to the web site
...
def extractSubtitlefromVideo(videoname, featurefilename=None, concurrency=10, subformat='srt', api_key=API_KEY, secret_key=SECRET_KEY):
    ...
    try:
        subtitle_file_path = generate_subtitles(
            source_path=videoname,
            output=featurefilename,
